...
// import { initSocketIO } from "./socket"; // Taken from ask_easy, not used in this project
...
async function main(): Promise<void> {
  ...
  httpServer.listen(port, () => {
    console.log(
      `> Wrapper listening on http://${hostname}:${port} (${dev ? "development" : "production"})`,
      `\n> Devs can connect on http://localhost:${port}`,
    );
    // console.log(`> Socket.IO server attached`); // Taken from ask_easy, not used in this project
  });
}
