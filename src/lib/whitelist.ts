...
// which UTORids are instructors. One UTORid per line; comment lines start with #.
//
// Any UTORid in the file → INSTRUCTOR
// Any UTORid NOT in the file → STUDENT
//
// TAs are assigned per-course by instructors via the UI and stored in
...
/**
 * Returns INSTRUCTOR if the UTORid is in the whitelist, STUDENT otherwise.
 * TAs are never in the whitelist — they are managed per-course via the UI.
 */
export function getRoleFromWhitelist(utorid: string): CourseRole {
  return WHITELIST.has(utorid.toLowerCase()) ? "INSTRUCTOR" : "STUDENT";
}
