 ...
 *
 * How to run:
 *   npx tsx src/lib/tests/revert_to_waiting/revert-to-waiting.test.ts
 *
 * Scenarios covered:
...
async function main() {
  ...
  await runTest("IN_HELP student → reverted to WAITING, helpStartedAt and helpedByHostId cleared", async () => {
    await cleanupAll();
    ...
    assertEqual(updated?.status, "WAITING", "status should be WAITING after revert");
    assertEqual(updated?.helpStartedAt, null, "helpStartedAt should be cleared");
    assertEqual(updated?.helpedByHostId, null, "helpedByHostId should be cleared");
  });
  ...
  await runTest("Student already WAITING → returns null", async () => {
    ...
    assertEqual(publicId, null, "should return null when student is already WAITING");

    // Status should be unchanged
    const unchanged = await prisma.officeHourAttendance.findUnique({ where: { id: attendance.id } });
  ...
  await runTest("Resolved student has no active attendance → revertToWaiting with invalid id returns null", async () => {
    await cleanupAll();
    const { offering } = await setupOffering();
    const session = await setupSession(offering.id);
  ...
  await runTest("Reverting student A leaves student B IN_HELP untouched", async () => {
    await cleanupAll();
    const { offering } = await setupOffering();
    const session = await setupSession(offering.id);
    ...
    const attendanceA = await prisma.officeHourAttendance.create({
      data: { sessionId: session.id, studentId: studentA.id, status: "IN_HELP", checkedInAt: new Date(), helpStartedAt: new Date() },
    });
    const attendanceB = await prisma.officeHourAttendance.create({
      data: { sessionId: session.id, studentId: studentB.id, status: "IN_HELP", checkedInAt: new Date(), helpStartedAt: new Date() },
    });
  ...
  await runTest("After revert, student returns to correct rank based on original checkedInAt", async () => {
    await cleanupAll();
    const { offering } = await setupOffering();
    const session = await setupSession(offering.id);
    ...
    await prisma.officeHourAttendance.create({
      data: { sessionId: session.id, studentId: studentA.id, status: "WAITING", checkedInAt: new Date(now) },
    });
    const attendanceB = await prisma.officeHourAttendance.create({
      data: { sessionId: session.id, studentId: studentB.id, status: "WAITING", checkedInAt: new Date(now + 1000) },
    });
    await prisma.officeHourAttendance.create({
      data: { sessionId: session.id, studentId: studentC.id, status: "WAITING", checkedInAt: new Date(now + 2000) },
    });
