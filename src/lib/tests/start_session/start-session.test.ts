 ...
 *
 * How to run:
 *   npx tsx src/lib/tests/start_session/start-session.test.ts
 *
 * Scenarios covered:
...
async function main() {
  ...
  await runTest("SCHEDULED session → transitions to ACTIVE, returns 'started'", async () => {
    await cleanupAll();
  ...
  await runTest("DELAYED session → transitions to ACTIVE, returns 'started'", async () => {
    await cleanupAll();
    const { offering } = await setupOffering();
  ...
  await runTest("Already ACTIVE session → returns 'already_active', status unchanged", async () => {
    await cleanupAll();
    const { offering } = await setupOffering();
    ...
    const unchanged = await prisma.officeHourSession.findUnique({ where: { id: session.id } });
    assertEqual(unchanged?.status, "ACTIVE", "status should still be ACTIVE");
  });

  // ── Test 4: COMPLETED session → returns "invalid_state" ──────────────────
  await runTest("COMPLETED session → returns 'invalid_state', status unchanged", async () => {
    await cleanupAll();
    const { offering } = await setupOffering();
    ...
    const unchanged = await prisma.officeHourSession.findUnique({ where: { id: session.id } });
    assertEqual(unchanged?.status, "COMPLETED", "status should still be COMPLETED");
  });

  // ── Test 5: Concurrent start — only one call wins ─────────────────────────
  await runTest("Two concurrent startSession calls → only one returns 'started'", async () => {
    await cleanupAll();
    const { offering } = await setupOffering();
    const session = await createSession(offering.id, "SCHEDULED");
    ...
    const alreadyActiveCount = results.filter((r) => r === "already_active").length;

    assertEqual(startedCount, 1, "exactly one call should return 'started'");
    assertEqual(alreadyActiveCount, 1, "exactly one call should return 'already_active'");
  });
