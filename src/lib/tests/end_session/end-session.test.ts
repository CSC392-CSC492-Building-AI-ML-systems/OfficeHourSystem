 ...
 *
 * How to run:
 *   npx tsx src/lib/tests/end_session/end-session.test.ts
 *
 * Scenarios covered:
...
async function createSession(
  ...
  return prisma.officeHourSession.create({
    data: { offeringId, title: "Test Session", type: "REGULAR", startsAt: start, endsAt: end, status },
  });
}
...
async function main() {
  ...
  await runTest("ACTIVE session with no students → COMPLETED, returns 'ended'", async () => {
    await cleanupAll();
    ...
    const updated = await prisma.officeHourSession.findUnique({ where: { id: session.id } });
    assertEqual(updated?.status, "COMPLETED", "session status should be COMPLETED");
  });

  // ── Test 2: WAITING students → CANCELLED records, attendance deleted ──────
  await runTest("WAITING students → CANCELLED records created, attendance rows deleted", async () => {
    await cleanupAll();
    const { offering } = await setupOffering();
    const session = await createSession(offering.id);
    ...
    await prisma.officeHourAttendance.createMany({
      data: [
        { sessionId: session.id, studentId: s1.id, status: "WAITING", checkedInAt: new Date() },
        { sessionId: session.id, studentId: s2.id, status: "WAITING", checkedInAt: new Date() },
      ],
    });
    ...
    const remaining = await prisma.officeHourAttendance.count({ where: { sessionId: session.id } });
    ...
    assert(records.every((r) => r.outcome === "CANCELLED"), "all records should be CANCELLED");
    assert(records.every((r) => r.helpEndedAt === null), "helpEndedAt should be null for cancelled");
  });

  // ── Test 3: IN_HELP students → COMPLETED records, helpEndedAt null ────────
  await runTest("IN_HELP students → COMPLETED records, helpEndedAt null, attendance deleted", async () => {
    await cleanupAll();
    const { offering } = await setupOffering();
    const session = await createSession(offering.id);
    ...
    await prisma.officeHourAttendance.createMany({
      data: [
        { sessionId: session.id, studentId: s1.id, status: "IN_HELP", checkedInAt: new Date(), helpStartedAt: new Date() },
        { sessionId: session.id, studentId: s2.id, status: "IN_HELP", checkedInAt: new Date(), helpStartedAt: new Date() },
      ],
    });
    ...
    assertEqual(records.length, 2, "should have 2 records");
    assert(records.every((r) => r.outcome === "COMPLETED"), "all records should be COMPLETED");
    assert(records.every((r) => r.helpEndedAt === null), "helpEndedAt null — force-ended, not normal END HELP");
  });

  // ── Test 4: Mix of WAITING and IN_HELP → both resolved correctly ──────────
  await runTest("Mix of WAITING and IN_HELP → correct outcomes for each", async () => {
    await cleanupAll();
    const { offering } = await setupOffering();
    const session = await createSession(offering.id);
    ...
    await prisma.officeHourAttendance.createMany({
      data: [
        { sessionId: session.id, studentId: w1.id, status: "WAITING", checkedInAt: new Date() },
        { sessionId: session.id, studentId: w2.id, status: "WAITING", checkedInAt: new Date() },
        { sessionId: session.id, studentId: h1.id, status: "IN_HELP", checkedInAt: new Date(), helpStartedAt: new Date() },
      ],
    });
    ...
    assertEqual(cancelled.length, 2, "2 CANCELLED records for WAITING students");
    assertEqual(completed.length, 1, "1 COMPLETED record for IN_HELP student");
  });

  // ── Test 5: Already COMPLETED → returns "already_ended" ──────────────────
  await runTest("Already COMPLETED session → returns 'already_ended'", async () => {
    await cleanupAll();
    const { offering } = await setupOffering();
  ...
  await runTest("After endSession, student can check in again (attendance table cleared)", async () => {
    await cleanupAll();
    const { offering } = await setupOffering();
    const session = await createSession(offering.id);
    ...
    assert(!errorThrown, "student should be able to re-check in after session ended");
  });
