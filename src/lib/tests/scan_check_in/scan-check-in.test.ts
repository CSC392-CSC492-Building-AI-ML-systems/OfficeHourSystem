 ...
 *
 * How to run:
 *   npx tsx src/lib/tests/scan_check_in/scan-check-in.test.ts
 *
 * Scenarios covered:
 *   Happy path:
...
async function makeSession(offeringId: number, status: "ACTIVE" | "SCHEDULED" = "ACTIVE") {
  const now = new Date();
...
async function makeStudent(suffix: string, offeringId: number | null, role: "STUDENT" | "TA" = "STUDENT") {
...
async function main() {
  ...
  await runTest("student_number lookup → checked_in, studentName returned", async () => {
    await cleanupAll();
    ...
    const result = await scanCheckIn(session.id, offering.id, "student_number", student.studentNumber!);

    assertEqual(result.outcome, "checked_in", "outcome");
    assert("studentName" in result, "studentName present");
    assert((result as { studentName: string }).studentName.includes("snumA"), "studentName contains last name");
  ...
  await runTest("utorid lookup → checked_in", async () => {
    ...
    const result = await scanCheckIn(session.id, offering.id, "utorid", student.utorid);
  ...
  await runTest("barcode in mock map → mock_user, studentName from map", async () => {
    await cleanupAll();
    const offering = await makeOffering("103");
    const session  = await makeSession(offering.id);

    // No DB student needed — mock map is keyed by barcode string
    const result = await scanCheckIn(session.id, offering.id, "barcode", "1234567890123456");

    assertEqual(result.outcome, "mock_user", "outcome");
    assert("studentName" in result, "studentName present");
    assertEqual((result as { studentName: string }).studentName, "Mock Student", "studentName from mock map");
  });

  // ── 4. studentName falls back to publicId when name fields are empty ──────
  await runTest("student with no firstName/lastName → publicId used as studentName", async () => {
    await cleanupAll();
  ...
  await runTest("unknown student_number → student_not_found", async () => {
    ...
    const result = await scanCheckIn(session.id, offering.id, "student_number", "0000000000");
  ...
  await runTest("unknown utorid → student_not_found", async () => {
    ...
    const result = await scanCheckIn(session.id, offering.id, "utorid", "zzz00000");
  ...
  await runTest("barcode not in mock map → student_not_found", async () => {
    ...
    const result = await scanCheckIn(session.id, offering.id, "barcode", "0000000000000000");
  ...
  await runTest("student enrolled as TA (not STUDENT role) → not_enrolled", async () => {
    await cleanupAll();
    const offering = await makeOffering("109");
    const session  = await makeSession(offering.id);
    const ta       = await makeStudent("tainq", offering.id, "TA");

    const result = await scanCheckIn(session.id, offering.id, "utorid", ta.utorid);

    assertEqual(result.outcome, "not_enrolled", "outcome");
  });

  // ── 10. Student already in queue → already_in_queue ──────────────────────
  await runTest("student already in queue → already_in_queue (no duplicate row)", async () => {
    await cleanupAll();
    ...
    const first = await scanCheckIn(session.id, offering.id, "utorid", student.utorid);
    assertEqual(first.outcome, "checked_in", "first scan outcome");

    // Second scan — should detect duplicate
    const second = await scanCheckIn(session.id, offering.id, "utorid", student.utorid);
  ...
  await runTest("student resolved (in record table) then re-scans → checked_in", async () => {
    await cleanupAll();
  ...
  await runTest("student enrolled in offering A scanned for offering B → not_enrolled", async () => {
    await cleanupAll();
    ...
    const result = await scanCheckIn(sessionB.id, offeringB.id, "utorid", student.utorid);

    assertEqual(result.outcome, "not_enrolled", "outcome");
  });

  // ── 13. Two students check in independently ───────────────────────────────
  await runTest("two students check in independently — each gets their own row", async () => {
    await cleanupAll();
    ...
    const resultA = await scanCheckIn(session.id, offering.id, "utorid", studentA.utorid);
    const resultB = await scanCheckIn(session.id, offering.id, "utorid", studentB.utorid);
  ...
  await runTest("same student checks into two different sessions — no unique conflict", async () => {
    await cleanupAll();
    ...
    const r1 = await scanCheckIn(session1.id, offering.id, "utorid", student.utorid);
    const r2 = await scanCheckIn(session2.id, offering.id, "utorid", student.utorid);
